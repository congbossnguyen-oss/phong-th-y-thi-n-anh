...
describe("luanGiaiMenh — chạy trên lá bàn thật từ engine (không crash, dữ liệu sạch)", () => {
  it("lá 17:43 19/08/2026 chế độ Mệnh: sinh văn bản hợp lệ, không lộ undefined/NaN", () => {
    const laBan = lapLaBan({ nam: 2026, thang: 8, ngay: 19, gio: 17, phut: 43, cheDo: "menh" });
  ...
  it("chế độ 1080 (không có tứ trụ ngày) → hopLe=false, không throw", () => {
    const laBan = lapLaBan({ cheDo: "1080", soCuc: 7, amDuong: "+", hoaGiap: "Giáp Tý" });
    const kq = luanGiaiMenh(laBan);
    expect(kq.hopLe).toBe(false);
    expect(kq.theLinhVuc).toEqual([]);
  });

  it("canNgay = Giáp (Giáp luôn ẩn) → vẫn xác định được Mệnh Cung qua cung của Phù Đầu", () => {
    // 29/02/2028 12:00 có canNgay = Giáp (đã xác nhận qua API khi rà QA).
    const laBan = lapLaBan({ nam: 2028, thang: 2, ngay: 29, gio: 12, phut: 0, cheDo: "menh" });
  ...
  it("canNgay = địa bàn can RIÊNG của Trung cung (không khớp bất kỳ thienBanCan nào) → Mệnh Cung = Trung cung", () => {
    ...
    const laBan = lapLaBan({ nam: 1965, thang: 6, ngay: 15, gio: 8, phut: 30, cheDo: "menh" });
  ...
  it("rà nhiều năm sinh 1930-2026 (bước 7 năm): luôn xác định được Mệnh Cung (hopLe=true)", () => {
    const soLuot: { nam: number; hopLe: boolean; canNgay?: string }[] = [];
    for (let nam = 1930; nam <= 2026; nam += 7) {
      const laBan = lapLaBan({ nam, thang: 6, ngay: 15, gio: 10, phut: 0, cheDo: "menh" });
      const kq = luanGiaiMenh(laBan);
...
describe("luanGiaiMenhChiTiet — bản trả phí (Người thân / Giai đoạn cuộc đời / Cách cục nổi bật)", () => {
  it("lá 17:43 19/08/2026 chế độ Mệnh: đủ dữ liệu, không lộ undefined/NaN", () => {
    const laBan = lapLaBan({ nam: 2026, thang: 8, ngay: 19, gio: 17, phut: 43, cheDo: "menh" });
  ...
  it("mỗi mục Cách Cục nổi bật có tên + ý nghĩa hợp lệ (tra đúng bảng 81 tổ hợp)", () => {
    const laBan = lapLaBan({ nam: 2026, thang: 8, ngay: 19, gio: 17, phut: 43, cheDo: "menh" });
    const kq = luanGiaiMenhChiTiet(laBan);
  ...
  it("canNgay = địa bàn can riêng của Trung cung (ca đặc biệt 15/06/1965) → vẫn hợp lệ", () => {
    const laBan = lapLaBan({ nam: 1965, thang: 6, ngay: 15, gio: 8, phut: 30, cheDo: "menh" });
    const kq = luanGiaiMenhChiTiet(laBan);
    expect(kq.hopLe).toBe(true);
    expect(kq.giaiDoanCuocDoi.length).toBe(4);
  ...
  it("rà nhiều năm sinh 1930-2026 (bước 7 năm): luôn hopLe=true, không crash", () => {
    const ketQua: boolean[] = [];
    for (let nam = 1930; nam <= 2026; nam += 7) {
      const laBan = lapLaBan({ nam, thang: 9, ngay: 10, gio: 15, phut: 0, cheDo: "menh" });
      const kq = luanGiaiMenhChiTiet(laBan);
  ...
  it("chế độ 1080 (không tứ trụ) → hopLe=false, không throw", () => {
    const laBan = lapLaBan({ cheDo: "1080", soCuc: 3, amDuong: "-", hoaGiap: "Ất Hợi" });
    const kq = luanGiaiMenhChiTiet(laBan);
    expect(kq.hopLe).toBe(false);
  });
});
