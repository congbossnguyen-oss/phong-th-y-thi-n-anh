...
export const POST: APIRoute = async ({ request, locals, clientAddress }) => {
  ...
  try {
    ...
    // cấp lên Cao cấp sau này xem lại lịch sử thì mở khoá được luôn cả phần cũ. Lỗi lưu KHÔNG được
    // làm hỏng response — khách đã tốn 1 lượt cho AI luận xong, lỗi ghi log lịch sử không đáng để
    // huỷ cả kết quả họ đang chờ.
    try {
