 ...
 * Kết quả (tính toán thuần công thức, không AI) do TRANG server-render khi khách vào lại với
 * ?orderCode=… (đã confirmed).
 */
export const GET: APIRoute = async ({ url, request, clientAddress }) => {
  const limited = checkRateLimit({ request, clientAddress }, { key: "result-trach-nhat-sinh-no-qs", max: 60, windowMs: 60_000 });
