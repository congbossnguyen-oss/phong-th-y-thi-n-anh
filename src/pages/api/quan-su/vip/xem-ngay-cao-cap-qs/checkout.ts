// Bản ĐỘC LẬP cho app Quân Sư của "Xem Ngày Cao Cấp" (anh Công chốt 1/9/2026: Quân Sư không đấu
// nối code với web nữa, kể cả module đã dùng chung trước đó). File này CỐ Ý gần như y hệt
// `src/pages/api/dai-cat-loi/xem-ngay-cao-cap/checkout.ts` — chấp nhận trùng lặp thay vì import
// chéo sang namespace `dai-cat-loi`. Chỉ khác: `TOOL_SLUG` riêng (hậu tố `-qs`) — vẫn dùng CHUNG
// engine tính toán (`calculateXemNgayCaoCap`) và hạ tầng thanh toán (`taoDonCongCu`).
...
const TOOL_SLUG = "xem-ngay-cao-cap-qs";
...
export const POST: APIRoute = async ({ request, locals, clientAddress }) => {
  const limited = checkRateLimit({ request, clientAddress }, { key: "checkout-xncc-qs", max: 10, windowMs: 60_000 });
