// Bản ĐỘC LẬP cho app Quân Sư của "Trạch Cát Kỳ Môn" (anh Công chốt 1/9/2026: Quân Sư không đấu
// nối code với web nữa, kể cả module đã dùng chung trước đó). File này CỐ Ý gần như y hệt
// `src/pages/api/dai-cat-loi/trach-cat-ky-mon/checkout.ts` — chấp nhận trùng lặp thay vì import
// chéo sang namespace `dai-cat-loi`. Chỉ khác: `TOOL_SLUG` riêng (hậu tố `-qs`) — vẫn dùng CHUNG
// engine tính toán (`trachCat`) và hạ tầng thanh toán (`taoDonCongCu`).
...
/**
 * Tạo đơn cho module Trạch Cát Kỳ Môn (bản app Quân Sư) — chọn ngày giờ tốt theo bàn Kỳ Môn Mệnh
 * của chính chủ sự. Tài khoản quản trị vẫn được taoDonCongCu tạo đơn ở mức 0đ + tự xác nhận để
 * test trọn luồng — xem cờ `laQuanTri` bên dưới.
 */
export const POST: APIRoute = async ({ request, locals, clientAddress }) => {
  const limited = checkRateLimit({ request, clientAddress }, { key: "checkout-trach-cat-ky-mon-qs", max: 10, windowMs: 60_000 });
