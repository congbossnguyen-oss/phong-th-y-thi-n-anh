...
export const GET: APIRoute = async ({ url, request, clientAddress }) => {
  const limited = checkRateLimit({ request, clientAddress }, { key: "result-trach-cat-ky-mon-qs", max: 60, windowMs: 60_000 });
  ...
  try {
    ...
    console.error(`[trach-cat-ky-mon-qs] Lỗi dựng kết quả cho đơn ${orderCode}:`, err);
    return jsonResponse({ ok: true, status: "confirmed", dangCapNhat: true }, 200);
  }
};
