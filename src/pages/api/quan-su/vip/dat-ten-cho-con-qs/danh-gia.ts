...
/**
 * Đánh giá danh sách tên khách tự chọn — CHỨC NĂNG THỨ HAI, mở chung bằng đúng đơn đã trả.
 * orderCode phải trỏ tới đơn `dat-ten-cho-con-qs` đã `confirmed`; thông tin sinh của bé lấy từ
 * snapshot của đơn (không tin client), danh sách tên (tối đa GIOI_HAN_DANH_GIA) lấy từ body.
 */
export const POST: APIRoute = async ({ request, clientAddress }) => {
  const limited = checkRateLimit({ request, clientAddress }, { key: "danh-gia-dat-ten-qs", max: 30, windowMs: 60_000 });
