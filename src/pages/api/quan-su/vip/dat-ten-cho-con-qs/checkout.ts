// Bản ĐỘC LẬP cho app Quân Sư của "Đặt Tên Cho Con Theo Việt Danh Học" (anh Công chốt 1/9/2026:
// Quân Sư không đấu nối code với web nữa, kể cả module đã dùng chung trước đó). File này CỐ Ý gần
// như y hệt `src/pages/api/dat-ten-cho-con/checkout.ts` — chấp nhận trùng lặp thay vì import chéo
// sang namespace web. Chỉ khác: `TOOL_SLUG` riêng (hậu tố `-qs`) — vẫn dùng CHUNG engine
// `@thien-anh/tinhdanh-engine` và hạ tầng thanh toán (`taoDonCongCu`).
...
const TOOL_SLUG = "dat-ten-cho-con-qs";
 ...
 * Một lần mua mở cả HAI chức năng (gợi ý tên + đánh giá tên có sẵn) cho cùng mã đơn.
 */
...
// Không bắt đăng nhập — khách nhập tên + SĐT liên hệ, orderCode làm "vé" mở kết quả.
export const POST: APIRoute = async ({ request, locals, clientAddress }) => {
  const limited = checkRateLimit({ request, clientAddress }, { key: "checkout-dat-ten-qs", max: 10, windowMs: 60_000 });
