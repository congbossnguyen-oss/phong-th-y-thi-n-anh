...
import { calculateNgayKhaiTruongCaoCap, type NgayKhaiTruongCaoCapInput } from "@thien-anh/trachnhat-engine";
...
// Trả kết quả sau khi đơn confirmed. Tính lại từ input đã lưu (deterministic). orderCode làm "vé".
export const GET: APIRoute = async ({ url, request, clientAddress }) => {
  const limited = checkRateLimit({ request, clientAddress }, { key: "result-khai-truong-cc-qs", max: 60, windowMs: 60_000 });
  ...
  try {
    const input = JSON.parse(order.toolInputSnapshot) as NgayKhaiTruongCaoCapInput;
    return jsonResponse({ ok: true, status: "confirmed", result: calculateNgayKhaiTruongCaoCap(input) }, 200);
  } catch (err) {
