...
// hệt `src/pages/api/dai-cat-loi/ngay-cuoi-hoi/checkout.ts` — chấp nhận trùng lặp thay vì import
// chéo sang namespace `dai-cat-loi`. Chỉ khác: `TOOL_SLUG` riêng (hậu tố `-qs`, khai trong
// `_chung.ts` cùng thư mục) — vẫn dùng CHUNG engine tính toán (`calculateCuoiHoiTronGoi`) và hạ
...
/**
 * Tạo đơn cho dịch vụ Cưới Hỏi trọn gói (bản app Quân Sư, 500.000đ).
 *
...
export const POST: APIRoute = async ({ request, locals, clientAddress }) => {
  const limited = checkRateLimit({ request, clientAddress }, { key: "checkout-cuoi-hoi-qs", max: 10, windowMs: 60_000 });
