// Bản ĐỘC LẬP cho app Quân Sư của "Chọn Giờ Liệm – Hạ Huyệt" (anh Công chốt 1/9/2026: Quân Sư
// không đấu nối code với web nữa, kể cả module đã dùng chung trước đó). File này CỐ Ý gần như y
// hệt `src/pages/api/dai-cat-loi/gio-liem-ha-huyet/checkout.ts` — chấp nhận trùng lặp thay vì
// import chéo sang namespace `dai-cat-loi`. Chỉ khác: `TOOL_SLUG` riêng (hậu tố `-qs`) — vẫn dùng
// CHUNG engine tính toán (`calculateGioLiemHaHuyet`, `kiemDayDuTruocThanhToan`) và hạ tầng thanh
// toán (`taoDonCongCu`, `giaGioLiemHaHuyet`).
...
export const POST: APIRoute = async ({ request, locals, clientAddress }) => {
  const limited = checkRateLimit({ request, clientAddress }, { key: "checkout-gio-liem-qs", max: 10, windowMs: 60_000 });
