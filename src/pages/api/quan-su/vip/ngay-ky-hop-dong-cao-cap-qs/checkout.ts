// Bản ĐỘC LẬP cho app Quân Sư của "Ngày Ký Hợp Đồng cao cấp" (anh Công chốt 1/9/2026: Quân Sư
// không đấu nối code với web nữa, kể cả module đã dùng chung trước đó). File này CỐ Ý gần như y
// hệt `src/pages/api/dai-cat-loi/ngay-ky-hop-dong-cao-cap/checkout.ts` — chấp nhận trùng lặp thay
// vì import chéo sang namespace `dai-cat-loi`. Chỉ khác: `TOOL_SLUG` riêng (hậu tố `-qs`) — vẫn
// dùng CHUNG engine tính toán (`calculateNgayKyHopDongCaoCap`) và hạ tầng thanh toán
// (`taoDonCongCu`).
...
/**
 * Tạo đơn cho module Ngày Ký Hợp Đồng cao cấp (bản app Quân Sư, 300.000đ).
 *
 * KHÔNG bắt đăng nhập, kết quả truy cập bằng orderCode làm "vé". Nếu khách tình cờ đang đăng nhập
 * thì gắn đơn vào tài khoản để họ xem lại được.
 */
export const POST: APIRoute = async ({ request, locals, clientAddress }) => {
  const limited = checkRateLimit({ request, clientAddress }, { key: "checkout-ky-hd-qs", max: 10, windowMs: 60_000 });
